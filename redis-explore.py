...
class RedisConnect(GUIApplication.GUIApplication):
    ...
    def create_widgets(self, root):
        ...
        self.database_label = tkinter.Label(self.root, text='Database:')
        self.database_label.grid(column=0, row=4, sticky=tkinter.E)
    ...
    def cmd_connect(self, evt=None):
        try:
            ...
            databases = ['db' + str(d) for d in range(number_databases)]
...
class RedisExplorer(GUIApplication.GUIApplication):
    ...
    def create_widgets(self, root):
        self.key_list, self.key_list_scrolled = self.create_scrolled(
            self.root, tkinter.Listbox, True, True)
        self.key_list_scrolled.grid(column=0, row=1, sticky=tkinter.NSEW)
        self.key_list.bind('<<ListboxSelect>>', self.cmd_select_key)

        self.object_frame = tkinter.Frame(root, bg='lightgrey')
        self.object_frame.grid(column=1, row=0, rowspan=2, sticky=tkinter.NSEW)

        self.object_delete = tkinter.Button(self.object_frame, width=10)
        self.object_delete.config(text='Delete', command=self.cmd_delete)
        self.object_delete.grid(column=1, row=0, sticky=tkinter.E)
        self.object_set_as = tkinter.Button(self.object_frame, width=10)
        self.object_set_as.config(text='Set As ...', command=self.cmd_set_as)
        self.object_set_as.grid(column=2, row=0, sticky=tkinter.E)
        self.object_set = tkinter.Button(self.object_frame, width=10)
        ...
        self.object_view_scrolled.grid(
            column=0, row=2, columnspan=5, sticky=tkinter.NSEW)
    ...
    def cmd_set(self):
        ...
        value = self.object_view.get(1.0, tkinter.END)[:-1]
